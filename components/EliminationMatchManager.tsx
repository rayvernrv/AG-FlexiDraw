...
export const EliminationMatchManager: React.FC = () => {
    ...
    const handleClearData = () => {
        ...
        if (confirm('Are you sure you want to clear the results and categories for the current round?')) {
            setRoundCategories(prev => {
                const next = { ...prev };
                delete next[currentRoundIndex];
                return next;
            });
            setResults(prev => {
                const next = { ...prev };
                activeSchedule?.matchups.forEach(m => {
                    delete next[m.id];
                });
                return next;
            });
